...
$(RAW_DUMPS_DIR)/construct_all.owl: $(RAW_DUMPS_DIR)/all.ttl
	$(call log, $@, $(ROBOT) merge -i $< reason --reasoner HERMIT --axiom-generators "SubClass EquivalentClass ClassAssertion" --exclude-tautologies structural relax annotate --ontology-iri "http://virtualflybrain.org/data/VFB/OWL/raw/all.owl" convert -f owl -o $@ $(STDOUT_FILTER))
